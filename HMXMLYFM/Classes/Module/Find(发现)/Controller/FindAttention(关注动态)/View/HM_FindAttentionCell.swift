...
class HM_FindAttentionCell: UITableViewCell {
    
    // 数据
    private var eventInfos: HM_EventInfosModel?
    ...
    // 评论
    lazy var commnetLabel : HM_CustomLabel = {
    ...
    lazy var collectionView : UICollectionView = {
        ...
        collectionView.register(HM_FindAttentionPicCell.self, forCellWithReuseIdentifier:"HM_FindAttentionPicCell")
    ...
    func setupView() {
        ...
        self.collectionView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo((SCREEN_WIDTH - 30) / 3 + 20)
        }
    }
    
    
    var eventInfosModel:HM_EventInfosModel? {
        didSet {
    ...
    func height(for commentModel: HM_FindAContentInfo?) -> CGFloat {
        ...
        height += label.sizeThatFits(CGSize(width: SCREEN_WIDTH - 30, height: CGFloat.infinity)).height
        return height
    }
...
extension HM_FindAttentionCell: UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
         return self.eventInfos?.contentInfo?.picInfos?.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: HM_FindAttentionPicCell = collectionView.dequeueReusableCell(withReuseIdentifier: "HM_FindAttentionPicCell", for: indexPath) as! HM_FindAttentionPicCell
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
       return CGSize(width: (SCREEN_WIDTH - 30) / 3, height: (SCREEN_WIDTH - 30) / 3)
    }
