...
class HM_PlayCell: UICollectionViewCell {
    var playUrl:String?
    var timer: Timer?
    ...
    private lazy var slider:UISlider = {
        ...
        slider.minimumTrackTintColor = BTN_COLOR
        // 滑块滑动停止后才触发ValueChanged事件
        //        slider.isContinuous = false
        
        slider.addTarget(self, action: #selector(HM_PlayCell.change(slider:)), for: UIControlEvents.valueChanged)
        
        slider.addTarget(self, action: #selector(HM_PlayCell.sliderDragUp(sender:)), for: UIControlEvents.touchUpInside)
        return slider
    }()
    // 当前时间
    ...
    func setupView(){
        ...
        self.imageView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(SCREEN_HEIGHT * 0.7 - 260)
        }
        // 弹幕按钮
