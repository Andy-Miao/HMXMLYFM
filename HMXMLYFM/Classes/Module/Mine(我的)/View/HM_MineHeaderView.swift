...
protocol HM_MineHeaderViewDelegate: NSObjectProtocol {
    func shopBtnClick(tag: Int)
}
class HM_MineHeaderView: UIView {

    weak var delegate: HM_MineHeaderViewDelegate?
    
    private lazy var animationView: HM_VipAnimationView = {
        let view = HM_VipAnimationView()
        return view
    }()
    ...
    private lazy var lineView:UIView = {
        let view = UIView()
        view.backgroundColor = SYC_Tools.RGBColor(r: 240, g: 240, b: 240)
        return view
    }()
    ...
    // 我的页面顶部视图的购买等按钮
    func setupShopView(){
        let margin:CGFloat = SCREEN_WIDTH / 10
